...
import {
  ...
  SkillLoadError,
  createSkillLoader,
} from "../../../src/platform/skills/loader";
import type { SkillConfig } from "../../../src/platform/skills/types";
...
`,
      ...
      expect(instance.config.requires.skills).toHaveLength(2);
      expect(instance.config.requires.skills).toContain("file-tools");
      expect(instance.config.requires.skills).toContain("base-skill");
    });

    it("应该正确解析空的 requires 字段", async () => {
      await writeFile(
        join(testDir, "skill.yaml"),
        `
id: skill-no-deps
...
`,
      ...
      expect(instance.config.requires).toBeDefined();
      expect(instance.config.requires.skills).toHaveLength(0);
    });

    it("应该处理没有 requires 字段的 skill", async () => {
      await writeFile(
        join(testDir, "skill.yaml"),
        `
id: skill-optional-deps
name: 可选依赖技能
version: "1.0.0"
type: knowledge
trigger: {}
`,
      ...
      // requires 是可选字段，不存在时应该是 undefined
      expect(instance.config.requires).toBeUndefined();
    });
  });
