...
describe("Config Loader", () => {
  beforeEach(() => {
    vi.resetAllMocks();
    vi.stubEnv("ZHIPU_ENDPOINT", "https://api.anthropic.com");
  ...
  describe("loadConfig", () => {
    it("L-01: should load valid YAML config", async () => {
      ...
      const { loadConfig } = await import("@/platform/models/loader");
    ...
    it("L-03: should throw on invalid YAML syntax", async () => {
      ...
      const { loadConfig, ConfigLoadError } =
        await import("@/platform/models/loader");
...
`;
      ...
      // 验证错误会被抛出（可能是 ValidationError 被包装为 ConfigLoadError）
      await expect(loadConfig("/path/to/config.yaml")).rejects.toThrow();
    });
    ...
    it("L-06: should handle missing config file", async () => {
      ...
      const { loadConfig, FileNotFoundError } =
        await import("@/platform/models/loader");
  ...
  describe("loadConfigFromString", () => {
    it("L-11: should parse config from string", async () => {
      const { loadConfigFromString } = await import("@/platform/models/loader");
