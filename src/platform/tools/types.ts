 ...
 * 工具调用结果（带回退信息）
 */
export interface ToolCallResultWithFallback {
  /** 是否成功 */
  readonly success: boolean;
  /** 输出内容 */
  readonly output: unknown;
...
export interface ToolExecutionRouter {
   ...
   *   [
   *     { tool: 'tavily-search', args: { query: 'AI news' } },
   *     { tool: 'exa-search', args: { query: 'AI news' } }
   *   ]
   * );
   */
  executeWithFallback(
    primary: ToolCallRequest,
    fallbacks: ToolCallRequest[],
    options?: FallbackOptions,
  ): Promise<ToolCallResultWithFallback>;
   ...
   * @returns 最佳工具名称
   */
  selectBestTool(tools: string[], context: ToolContext): string;
   ...
   * 记录工具调用结果
   */
  recordToolResult(toolName: string, success: boolean, latency: number): void;
}
...
export interface FallbackOptions {
  ...
  readonly timeout?: number;
  /** 回退回调 */
  readonly onFallback?: FallbackCallback;
}
