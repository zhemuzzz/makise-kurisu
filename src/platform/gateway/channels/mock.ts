...
export class MockChannel extends BaseChannel {
  ...
  async handleRequest(req: unknown, res: unknown): Promise<void> {
    ...
    if (this.mockConfig.echo) {
      reply = content;
    } else if (this.mockConfig.gateway) {
      try {
        const result = await this.mockConfig.gateway.processStream(
          inbound.sessionId,
          inbound.content,
          inbound.userId,
        );
        reply = await result.finalResponse;
      } catch (error) {
        console.error("MockChannel Gateway error:", error);
