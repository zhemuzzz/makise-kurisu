...
export class YamlConfigLoader implements IConfigLoader {
  ...
  loadFromString(content: string): ModelsYamlConfig {
    ...
    try {
      ...
      throw new ConfigLoadError('Failed to parse YAML', error as Error);
    }
    ...
    if (config.models) {
      ...
      for (const model of config.models) {
        ...
        if (names.has(model.name)) {
          console.warn(`[ConfigLoader] Warning: Duplicate model name: ${model.name}`);
        }
  ...
  private validateModel(model: ModelConfig): void {
    ...
    if (!model.type || !['local', 'cloud', 'api'].includes(model.type)) {
      throw new ValidationError(`Model "${model.name}" has invalid type: ${model.type}`);
    }

    if (!model.provider || typeof model.provider !== 'string') {
      throw new ValidationError(`Model "${model.name}" must have a valid "provider" field`);
    }
  }
}
 ...
 * 便捷函数：从文件加载配置
 */
export async function loadConfig(configPath: string): Promise<ModelsYamlConfig> {
